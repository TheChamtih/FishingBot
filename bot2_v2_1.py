...
def create_green_mask_fast(roi_bgr):
    """
    СУПЕР-БЫСТРАЯ векторизация через cv2.inRange (в 2-3 раза быстрее NumPy!)
    Создает маску зеленых пикселей
    """
    # Два диапазона: темно-зеленый (мини-игра 1) и ярко-зеленый (мини-игра 2).
    # Добавляем доминирование G над R/B для отсечения нейтральных бликов и воды.
...
def find_horizontal_bar(screen_bgr):
    ...
    for y in range(4, roi_h - 4, 2):
        ...
        for s, e in zip(starts, ends):
            seg_len = int(e - s + 1)
            if seg_len < 70 or seg_len > 420:
                continue

            # Ищем компактный белый кластер внутри зеленого сегмента.
            # Курсор обычно дает более широкий/рваный след.
            segment_white = white_band[s:e + 1]
            ...
            for ws, we in zip(w_starts, w_ends):
                run_len = int(we - ws + 1)
                # Маркер-крючок обычно узкий в проекции по X.
                if run_len < 1 or run_len > 14:
                    continue

                wx = int(s + (ws + we) // 2)
                dist_center = abs(wx - seg_center)
                run_score = 100 - dist_center - run_len
            ...
            if white_x - s < 4 or e - white_x < 4:
                continue

            left_red = np.any(red_band[max(0, s - 28):min(roi_w, s + 14)])
            right_red = np.any(red_band[max(0, e - 14):min(roi_w, e + 28)])
            if not (left_red and right_red):
                continue

            score = 0
            score += min(seg_len, 220)
            score -= abs(((s + e) // 2) - center_x) * 2
...
def check_hook_in_green(screen_bgr, hook_pos):
    ...
    # Нужно одновременно: геометрия сегмента + заметная доля зеленого + слабый красный в центре.
    local_green_ok = (
        (center_green_ratio >= 0.10 and center_green_ratio >= center_red_ratio * 1.15) or
        (center_green_ratio >= 0.18 and center_red_ratio < 0.16)
    )
    in_green = in_green_segment and local_green_ok and (red_center_ratio < 0.24)
...
with mss.mss() as sct:
    while True:
        ...
        if hook_pos:
            ...
            click_candidate = in_green and (8 <= bar_width <= 220)
            if click_candidate:
            ...
            high_confidence_green = in_green and green_ratio >= 0.32 and (12 <= bar_width <= 220)
